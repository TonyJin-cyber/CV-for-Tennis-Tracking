...
#run :  python -m scripts.test --camera_npz outputs/calib/camera_1080p.npz --H_npz outputs/calib/H_und.npz --weights models/yolo11/yolo11_tennis.pt --cam 0 --conf 0.35 --show
...
def main():
    ...
    while True:
        ...
        for x1, y1, x2, y2, c in boxes:
            ...
            vis_boxes.append((x1, y1, x2, y2, c, x, y))
        ...
        if args.show:
            ...
            num_balls = len(landmark_map.active())

            panel_h, panel_w = 60, 220
            cv2.rectangle(vis, (0, 0), (panel_w, panel_h), (0, 0, 0), -1)

            cv2.putText(
                vis,
                f"Balls (unique): {num_balls}",
                (10, 40),
            ...
            if target_xy is not None:
                cv2.putText(
                    vis,
                    f"Target (x,y): ({target_xy[0]:.2f}, {target_xy[1]:.2f}) m",
                    (10, 65),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.6,
                    (255, 255, 255),
            ...
            for x1, y1, x2, y2, c, gx, gy in vis_boxes:
                color = (0, 255, 0)  # default: green

                if target_xy is not None:
                    dist = np.hypot(gx - target_xy[0], gy - target_xy[1])
                    if dist < 0.15:  # 15 cm 阈值，可调
                        color = (0, 0, 255)  # target: red
