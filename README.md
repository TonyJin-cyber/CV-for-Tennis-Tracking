...
1. **脚本重复度高：`scripts/test.py` 与 `scripts/run_camera_pipeline.py` 结构高度重复**  
   `test.py` 主要是在可视化上做少量增强（计数面板、目标高亮），其余主流程几乎一致。建议：
   - 合并为一个脚本，通过参数开关控制“简洁/增强可视化模式”；或
   - 将公共逻辑提取到 `core/pipeline.py`，两个入口只保留差异化 UI。

2. **`modules/detect_yolo.py` 当前为占位实现（TODO 未完成）**  
   该模块目前返回空检测，与主流程中实际使用的 `Detector` 类（写在脚本内）形成“重复但不统一”的实现。建议：
   - 把 `run_camera_pipeline.py` 的 `Detector` 挪到 `modules/detect_yolo.py`；
   - 所有脚本统一调用模块化接口。

3. **`run_demo.py` 与主流水线存在功能重叠**  
   `run_demo.py` 是更早期、较独立的版本，几何流程与检测逻辑与主流水线重复。建议：
   - 若仅用于教学，可保留并在 README 标注“legacy demo”；
   - 若用于生产，建议下线或改为调用主模块，避免双份维护。

4. **历史产物不应纳入版本库：`__pycache__/*.pyc`**  
   这些是运行时缓存，不属于源码资产。已在本次整理中移除并通过 `.gitignore` 忽略。

5. **缺少依赖锁定文件**  
   不是“冗余”但会带来复现成本。建议补充 `requirements.txt` 或 `pyproject.toml`。

---

## 7. 建议的下一步重构路线

1. 新建 `core/pipeline_runner.py`：抽取检测、映射、更新地图等公共逻辑。  
2. `scripts/run_camera_pipeline.py` 和 `scripts/test.py` 仅保留 CLI 与可视化差异。  
3. 完成 `modules/detect_yolo.py`，移除脚本内 Detector 以统一入口。  
4. 增加 `requirements.txt` 与最小化自测脚本（如读取样例帧跑一次前向）。
...
- ✅ 增加 README（本文件）
- ✅ 清理已追踪的 `__pycache__` 二进制缓存
- ✅ 新增 `.gitignore`，避免缓存再次入库
